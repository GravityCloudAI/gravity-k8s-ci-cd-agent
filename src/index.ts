			...
			await Promise.all(services.map(async (service: any) => {
				try {
					...
					const dockerBuildCommand = process.env.ENV === "production"
						? `${dockerBuildCli} bud --isolation chroot --platform=linux/amd64 --tls-verify=false --layers --cache-from ${localRegistryUrl}/${owner}/${serviceName}/cache --cache-to ${localRegistryUrl}/${owner}/${serviceName}/cache -t ${owner}/${serviceName}:latest -f ${dockerfilePath} ${serviceContext}`
